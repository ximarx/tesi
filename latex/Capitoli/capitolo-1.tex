...
%Esiste una grande varietà di tecniche che permettono di approcciare il problema dello sviluppo di sistemi esperti. \cite{jackson1999}
...
Il processo di elaborazione di simboli e strutture simboliche rappresentanti varie concetti e relazioni fra gli stessi è definito \emph{computazione simbolica}.
...
Gli strumenti di sviluppo per sistemi esperti sono orientati tutti quanti al supporto alla creazione di \emph{prototipi}. Un prototipo rappresenta un modello funzionante le cui funzionalità sono equivalenti ad una porzione ridotta di quelle del prodotto finale. \cite{jackson1999}

L'idea è quella di sviluppare, nelle prime fasi del progetto, dei ''\emph{proof of concept}'' che possono essere valutati e criticati da esperti e utenti e che siano in grado di risolvere parti del problema. Questa attività è utilizzata soprattutto come un'opportunità per definire meglio requisiti e individuare criticità, verificando inoltre l'effettiva trattabilità di problematiche prima di effettuare investimenti eccessivi.
...
%Un problema derivante dallo sviluppo incrementale di sistemi esperti tramite prototipi successivi consiste nell'attività di integrazione delle nuove funzionalità sviluppate all'interno del prototipo rappresentante la versione precedente.

%Per risolvere questo genere di problematica, gli ambienti di sviluppo per sistemi esperti forniscono la possibilità di rappresentare la conoscenza in forme partizionate in moduli indipendenti.
...
%I primi strumenti erano semplici metodi di rappresentazione della conoscenza basata sull'uso delle regole, con il supporto di strumenti di inferenza piuttosto limitati. Con gli anni, la lista e la tipologia di funzionalità offerte si è arricchita introducendo una varietà di possibilità per la rappresentazione della conoscenza, strategie di ricerca, meccanismi specializzati di inferenza e strumenti di supporto allo sviluppo dei sistemi.
...
Lo sviluppo si è focalizzato in molte aree di interesse, spaziando dall'ambito delle modalità di rappresentazione della conoscenza a quello dei meccanismi di inferenza, dallo sviluppo di regimi controllo alla ricerca nell'ambito dei linguaggi di specifica. Ma indipendentemente dalle modalità con i quali è stato tentato il miglioramento, lo slancio evolutivo era ad ogni modo focalizzati nell'ambito di:
\begin{itemize}
...
Tornando indietro agli albori della produzione di sistemi esperti, i meccanismi di inferenza e le basi di conoscenza erano strettamente accoppiati fra loro. La prima generazione di sistemi non era altro che grandi applicazioni scritte in linguaggi come LISP.~\cite{knowbel1993} 

Nonostante tutti i linguaggi di programmazione possano essere utilizzati per la produzione di sistemi esperti, alcuni di essi offrivano caratteristiche che rendevano la stessa attività più agevole. Rientrano in questa categoria linguaggi largamente utilizzati nelle prime fasi come il LISP e Prolog. 
...
Strumenti che offrivano maggiore flessibilità offrivano solo meccanismi di rappresentazione e \emph{problem-solving} basilari. Questo li rendeva adatti a qualsiasi ambito e problematica, ma il basso livello di sofisticazione richiedeva agli ingegneri della conoscenza di specificare personalmente parte dei meccanismi di ragionamento e di rappresentazione. Esempi di questa sottoclasse di strumenti sono ROSIE, OPS5 e Prolog\footnote{Sebbene il Prolog sia un linguaggio di programmazione logica general-purpose, la disponibilità di meccanismi di ragionamento basilari basati su un regime di tipo \emph{backward-chaining} e la forma dichiarativa del linguaggio che rendeva agevole la rappresentazione della conoscenza lo rendono ascrivibile anche alla categoria delle \emph{General-Purpose Shell}}.

Maggiore semplicità era quella offerta invece dai sistemi come BLOBS, G2 o CubiCal~\cite{development1993}. Il loro ambito di utilizzo veniva ridotto, ma il maggiore livello di sofisticazione nelle forme ragionamento unito alla disponibilità di paradigmi multipli per la rappresentazione della conoscenza rendeva più agevole lo sviluppo di sistemi esperti. Inoltre, agli ingegneri della conoscenza era richiesto una minore conoscenza nell'ambito delle dinamiche del sistema di base. La modifica dei meccanismi di ragionamento era ancora possibile, anche se non obbligatoria.

Gli strumenti posti al più alto grado di sofisticazione nella categoria degli \emph{environment} erano progettati per affrontare più aree di problemi. Offrivano paradigmi multipli per la specifica della conoscenza, del ragionamento unite a caratteristiche e utilità orientate allo sviluppo dei sistemi esperti ed ridurre i tempi di iterazione nei processi di prototipazione. 

La presenza di interpreti già integrati e di paradigmi multipli di rappresentazione rendevano il carico degli ingegneri della conoscenza nella definizione dei nuovi sistemi estremamente ridotto. Nonostante tutto, la scelta di utilizzare alcune di queste utilità offerte portava ad influenzare fortemente il design nei casi in cui i paradigmi offerti non corrispondessero perfettamente ai requisiti.~\cite{development1993}
...
Nonostante l'implementazione fosse fortemente incentrata sul motore inferenziale di tipo \emph{forward}, permetteva anche ragionamento basato sulla concatenazione all'indietro. L'utilizzo del regime alternativo era però sottoposto alla necessità di una specifica esplicita di strutture di controllo, la definizione di priorità e obiettivi.
...
Il sistema è liberamente utilizzabile per uso personale ed accademico, mentre l'uso commerciale è soggetto all'acquisto di una licenza personale \emph{una tatum}.

Lo sviluppo del progetto, nonostante sia attualmente affidato unicamente ad una sola persona, è ancora in atto. Il codice sorgente è disponibile pubblicamente, insieme alle versioni compilate.
...
Nelle versioni successive, giustificate dal successo del progetto sia come \emph{training-tool} prima, che come reale ambiente di sviluppo e distribuzione di sistemi esperti più tardi, il progetto è stato esteso con l'integrazione del paradigma procedurale e ad oggetti (COOL).
...
Negli anni diversi sistemi che espandevano specifiche categorie di caratteristiche sono stati derivati dal progetto originale. Alcuni esempi sono ECLiPSe~\cite{haley1991}, eCLIPS~\cite{eclips} e FuzzyCLIPS~\cite{fuzzyclips}.
...
La scelta di molti durante questo processo di conversione ricadde verso il linguaggio C, molto diffuso e supportato da tutte le piattaforme. L'adozione del C portò un ad incremento prestazione dei sistemi oltre ad offrire nuove scenari in cui sperimentare l'integrazione fra il software disponibile sulle nuove piattaforme e gli strumenti di sviluppo per sistemi esperti.

Prendendo in esame CLIPS, le possibilità di integrazione offerte si basato sulla possibilità di integrare all'interno di un prodotto finito scritto in un linguaggio che consenta l'integrazione di librerie scritte in linguaggio C. JESS estende ulteriormente queste possibilità tramite l'utilizzo di un Java, linguaggio emergente negli ultimi anni sia in ambiente \emph{coorporate} che nei prodotti di largo consumo.
...
Sebbene questi studi abbiamo mostrato, come prevedibile, che le caratteristiche di particolari linguaggi si adattino meglio ad alcuni ambiti di utilizzo particolari rispetto che ad altri, un elemento comune emerso riguarda l'impatto che la scelta del linguaggio ha sui costi di sviluppo e l'affidabilità degli artefatti prodotti. Nonostante la grande flessibilità offerta dal linguaggio C, i costi sopportati per lo sviluppo, il testing e la manutenzione degli artefatti prodotti era superiore rispetto ad altri linguaggi come Ada~\cite{Zeigler_95}, Java~\cite{prashant2008} o a linguaggi interpretati come Perl o Python\cite{prechelt2000}.

\section{Obiettivi di questa tesi}
Negli ultimi anni, Internet e più specificatamente il \emph{World Wide Web} sta evolvendo da un media per lo scambio di informazioni verso una piattaforma onnipresente per la distribuzione di qualsiasi genere di servizi come il \emph{Web banking}, \emph{commercio online}, \emph{E-Government}, etc. La ragione alla base di questa rapida evoluzione del Web è il grande numero di possibilità che questa forma di distribuzione offre, unito al grande successo che questo genere di soluzioni sta riscontrando verso l'utenza.
...
In questo ambito, la possibilità di integrazione delle tecnologie classiche per la realizzazione di sistemi esperti con strumenti di nuova generazione orientati allo sviluppo per web sta aprendo una frontiera per gli \emph{expert system environment}.
