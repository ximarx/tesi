...
In seguito all'aggiunta di una nuova definizione di regola, l'insieme di pattern che compongono la porzione \emph{LHS} viene scomposto e analizzato singolarmente. Per ognuno di essi viene creato, o condiviso, un circuito \emph{alpha} in grado di descrivere le espressioni
...
Di maggiore interesse è invece l'osservazione dell'andamento del \emph{gap} prestazionale evidenziato dai test differenti (\figurename~\ref{fig:myclips-vs-clips-bench}). Il prototipo MyCLIPS si è rivelato fra le 20 e le 130 volte più lento rispetto al software CLIPS utilizzando istanze locali, mentre fra le 60 e 200 volte usando istanze remote tramite modulo server. Il divario si è rivelato essere più marcato con l'aumento delle dimensioni dei dati gestiti nella \emph{working-memory}, è rimasto invece costante per i test relativi al benchmark \emph{Sudoku}.
